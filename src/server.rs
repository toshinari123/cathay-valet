...
impl ChatServer {
    pub fn new() -> ChatServer {
        ...
        Self { sessions: HashMap::new(), rooms, rng: rand::thread_rng() }
    }
    fn send_message(&self, room: &str, message: &str, skip_id: usize) {
        if let Some(sessions) = self.rooms.get(room) {
            for id in sessions { if *id != skip_id { 
...
impl Handler<Connect> for ChatServer {
    ...
    fn handle(&mut self, msg: Connect, _: &mut Context<Self>) -> Self::Result {
        ...
        self.rooms.entry("main".to_string()).or_insert_with(HashSet::new).insert(id);
    ...
    fn handle(&mut self, msg: Disconnect, _: &mut Self::Context) -> Self::Result {
        ...
        if self.sessions.remove(&msg.id).is_some() {
            for (name, sessions) in &mut self.rooms {
                if sessions.remove(&msg.id) { rooms.push(name.to_owned()); }
            }
        }
        for room in rooms {
            self.send_message("main", &json!({
...
impl Handler<ListRooms> for ChatServer {
    ...
    fn handle(&mut self, _: ListRooms, _: &mut Self::Context) -> Self::Result {
        let mut rooms = vec![];
        for key in self.rooms.keys() { rooms.push(key.to_owned()); }
        MessageResult(rooms)
    }
}
...
impl Handler<Join> for ChatServer {
    ...
    fn handle(&mut self, msg: Join, _: &mut Self::Context) -> Self::Result {
        ...
        for (n, sessions) in &mut self.rooms {
            if sessions.remove(&id) { rooms.push(n.to_owned()); }
        }
        for room in rooms {
