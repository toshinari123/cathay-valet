...
async function signIn({ passengerid }) {
    try {
        const url = "http://localhost:8080/users/" + passengerid;
        let result = await fetch(url);
        return result.json();
    } catch (e) {
        return Promise.reject(e);
    ...
    const FormSignIn = ({ setAuth }) => {
        ...
        return (
            <form action="" className="mt-4 space-y-2" onSubmit={onSignIn}>
                <div>
                    <label className="text-sm font-light">Passenger ID</label>
                    <input required type="text" name="phone" placeholder="510892B000014EBB"
                        className="w-full px-4 py-2 mt-2 border rounded-md focus:outline-none focus:ring-1 focus:ring-blue-600" />
                </div>
                <div className="flex items-baseline justify-between">
                    <button type="submit"
        ...
        <div className={`${show ? '' : 'hidden'} bg-gradient-to-b from-orange-400 to-rose-400`}>
            <div className="flex items-center justify-center min-h-screen">
                <div className="px-8 py-6 mt-4 text-left bg-white  max-w-[400px] w-full rounded-xl shadow-lg">
                    <h3 className="text-xl text-slate-800 font-semibold">Enter Passenger ID</h3>
